...
export default function Trust() {
  return (
    <section className="scroll-mt-24 border-y border-slate-100 bg-slate-50/60 py-16 md:py-24">
      <Container>
          ...
          {trustCards.map((card, i) => (
            <Reveal key={card.title} delay={i * 90}>
              <div className="group h-full rounded-2xl border border-slate-200 bg-white p-6 shadow-soft transition-all duration-300 hover:-translate-y-1 hover:border-deep-200 hover:shadow-soft-lg">
                <span className="grid h-12 w-12 place-items-center rounded-xl bg-deep-900 text-white transition-colors group-hover:bg-accent">
                  <Icon name={card.icon} className="h-6 w-6" />
                </span>
                <h3 className="mt-5 text-lg font-bold text-deep-950">{card.title}</h3>
